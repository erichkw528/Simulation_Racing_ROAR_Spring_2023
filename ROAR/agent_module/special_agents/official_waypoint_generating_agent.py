...
class OfficialWaypointGeneratigAgent(Agent):
    def __init__(self, vehicle: Vehicle, agent_settings: AgentConfig, **kwargs):
        super().__init__(vehicle=vehicle, agent_settings=agent_settings, **kwargs)
        self.output_file_path: Path = self.output_folder_path / str(str(self.agent_settings.spawn_point_id) + ".txt")
    ...
    def run_step(self, sensors_data: SensorsData,
        ...
        super(OfficialWaypointGeneratigAgent, self).run_step(sensors_data=sensors_data,
                                                     vehicle=vehicle)
